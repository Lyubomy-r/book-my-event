...
public class MediaServiceImp implements MediaService {
  ...
  private static final List<String> ALLOWED_CONTENT_TYPES =
      List.of("image/jpeg", "image/webp", "image/svg+xml", "image/png");
  ...
  private void validateFile(MultipartFile file) throws IOException {
    ...
    if (file.getSize() > MAX_FILE_SIZE_BYTES) {
      throw new IOException(
          "File size exceeds the " + MAX_SIZE_MB + " limit: " + file.getOriginalFilename());
    }
  }
  ...
  private byte[] optimizeImage(MultipartFile file) throws IOException {
    ...
    if (optimizedImage.length > MAX_FILE_SIZE_BYTES) {
      throw new GeneralException(
          "Optimized image still exceeds the 1MB limit: " + file.getOriginalFilename(),
  ...
  //        if (!ALLOWED_CONTENT_TYPES.contains(file.getContentType())) {
  //            throw new GeneralException("Unsupported file type: " + file.getContentType(),
  ...
  public String updateImageById(MultipartFile image, String imageId) {
    ...
    try {
      ...
      if (optimizedImageBytes == null || optimizedImageBytes.length == 0) {
        throw new GeneralException(
            "Failed to optimize the uploaded image. No valid image data found.",
            HttpStatus.BAD_REQUEST);
      }
  ...
  //            log.info("{}::deleteEventImg. deleteById existing Event Images. {} ", clasName,
  // imageId);
  //        }
  //    }
